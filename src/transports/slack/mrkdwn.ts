...
function convertText(text: string): string {
  ...
  out = out.replace(
    /\*\*([^\n*]+)\*\*/g,
    (_m, inner: string) => `${BOLD_OPEN}${inner}${BOLD_CLOSE}`,
  );
  out = out.replace(
    /__([^\n_]+)__/g,
    (_m, inner: string) => `${BOLD_OPEN}${inner}${BOLD_CLOSE}`,
  ...
  out = out.replace(
    /~~([^\n~]+)~~/g,
    (_m, inner: string) => `${STRIKE_OPEN}${inner}${STRIKE_CLOSE}`,
  );

  // Headings have no equivalent; bold on its own line is the closest Slack gets.
  out = out.replace(
    /^ {0,3}#{1,6}\s+(.+?)\s*#*\s*$/gm,
    (_m, title: string) => `${BOLD_OPEN}${title}${BOLD_CLOSE}`,
  );
 ...
 */
function tablesToCodeBlocks(markdown: string): string {
  ...
  while (index < lines.length) {
    ...
    const looksLikeDivider =
      next !== undefined && /^\s*\|?[\s:|-]*-[\s:|-]*\|?\s*$/.test(next) && next.includes("-");
...
export function splitMessage(text: string, limit = 3800): string[] {
  ...
  for (const line of text.split("\n")) {
    ...
    current += (current ? "\n" : "") + line;
    if (line.trimStart().startsWith("```")) fenceOpen = !fenceOpen;
  }
