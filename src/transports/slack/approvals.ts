...
export interface PendingApproval {
  ...
  /** Set once the message asking the question has actually been posted. */
  channel?: string;
  messageTs?: string;
}
...
export class ApprovalRegistry {
  private readonly pending = new Map<
    string,
    { entry: PendingApproval; settle: (decision: ApprovalDecision) => void; timer: NodeJS.Timeout }
  >();

  open(
    request: ApprovalRequest,
    timeoutMs: number,
    onExpire: (entry: PendingApproval) => void,
  ): { id: string; decision: Promise<ApprovalDecision> } {
    const id = randomUUID();
    const entry: PendingApproval = { id, request };
  ...
  locate(id: string, channel: string, messageTs: string): void {
   ...
   */
  settle(id: string, decision: ApprovalDecision): PendingApproval | undefined {
    const found = this.pending.get(id);
